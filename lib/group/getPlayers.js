...
function getPlayers (group, rankIndex, role, page, body) {
  ...
  var pager = $('.paging_pagenums_container');
  var totalPages = pager.length > 0 ? parseInt(pager.text(), 10) : 1;
  if (group) {
...
exports.func = function (args) {
  ...
  function getPages (inputs, rankIndex, role, pages, totalPages) {
    ...
    return threaded({getPage: getPage, start: start, end: end});
  }
      ...
      (function (i) {
        ...
        jobs.push(generalRequest(opt)
          .then(function (response) {
            ...
            var totalPages = getPlayers(first !== -1 ? group : null, i, role, 1, body, all);
            group.totalPages.push({
              role: role,
              total: totalPages
            });
            var page = pages;
